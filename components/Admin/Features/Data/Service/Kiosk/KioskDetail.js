...
export default class KioskDetail extends Component {
  constructor(props) {
    super(props);
    this.state = {
      kioskId: this.props.match.params.id,
      item: {
        company: '',
        name: '',
        phone: '',
  ...
  componentDidMount() {
    axios.get(`/api/service/kiosk/${this.state.kioskId}`).then(res => {
      this.setState({
        item: res.data[0]
      });
  ...
  deleteHandler = () => {
    axios
      .delete(`/api/service/kiosk/${this.state.kioskId}`)
      .then(this.props.history.push('/admin/service/kiosk'));
  };
      ...
      <Container>
        ...
        <Body>
          ...
          <TitleWrapper>
            ...
            <SubTitle>
              <div className='tag'>담당자: {item.name}</div>
            </SubTitle>
            <SubTitle>
